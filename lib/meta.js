...
class Meta {
  ...
  static get FieldType() {
    if (Types.get('MetaFieldType')) {
      return Types.get('MetaFieldType');
    }
    ...
    Types.store('MetaFieldType', metaFieldType);
    ...
    if (typeSchema.fields && typeof typeSchema.fields === 'object') {
      for(let key of Object.keys(typeSchema.fields)) {
        ...
        if (Types.isOfType(fieldType, 'GraphQLObjectType')
          || Types.isOfType(fieldType, 'GraphQLInputObjectType')) {
          fieldMeta = Meta.get(field.type.toString());
        }
        // Check for Collection (array of objects)
        if (Types.isOfType(fieldType, 'GraphQLList')
          &&  (Types.isOfType(fieldType.ofType, 'GraphQLObjectType')
            || Types.isOfType(fieldType.ofType, 'GraphQLInputObjectType'))) {
        ...
        let metaField = {
          ...
          meta: fieldMeta,
          array: Types.isOfType(fieldType, 'GraphQLList'),
