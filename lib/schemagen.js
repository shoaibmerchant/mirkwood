...
import {
	...
	QueueUtility,
	DocumentStoreUtility } from '../utils';
...
class SchemaGenerator {
	...
	constructor({ config }) {
		this.utils = {
			...
			elasticsearch: new ElasticsearchUtility({ config: config.elasticsearch }),
			documentstore: new DocumentStoreUtility({ config: config.documentstore }),
			...
			queue: new QueueUtility({ config: config.queue })
		};
		this.orm = new ORM({ config: config.database });
	}
