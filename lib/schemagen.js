...
class SchemaGenerator {
	...
	_generateModelRelations(model) {
		...
		if (modelRelations.hasMany && Array.isArray(modelRelations.hasMany)) {
			modelFields['_children'] = {
				type: {
					fields: () => {
						let hasManyRelations = {};

						modelRelations.hasMany.forEach((relation) => {
							let resolverName = [modelKey, 'children', relation.key].join('.');

							hasManyRelations[relation.key] = this.utils.database.resolvers['join'](
								resolverName,
